...
public protocol SearchResultUseCase {
    ...
    func get(tags: [String], order: NovelListModel.Novel.SortField, limit: Int, offset: Int) -> Single<NovelListModel>
...
private struct SearchResultUseCaseImpl: SearchResultUseCase {
    ...
    func get(tags: [String], order: NovelListModel.Novel.SortField, limit: Int, offset: Int) -> Single<NovelListModel> {
        return self.novelListTranslator.convert(from: self.novelListRepository.get(tags: tags, order: order.rawValue, limit: limit, offset: offset))
    }
