...
private struct NovelListAPIDataStoreImpl: NovelListAPIDataStore {
    ...
    func get(text: String, order: String) -> Single<NovelListResponse> {
        return self.session.rx.response(for: NovelListRequest(text: text, order: order))
    }
    
    func get(genre: String, order: String) -> Single<NovelListResponse> {
        return self.session.rx.response(for: NovelListRequest(genre: genre, order: order))
    }
}
